# -*- coding: utf-8 -*-
"""
Created on Mon Nov 14 10:55:10 2022
...
def Sharpness (path_in,path_out):
  ...
  # Augmentation pipeline statement: Soft Sharpen

  transform = A.Compose([
          A.Sharpen (alpha=(0.2, 0.2), lightness=(1, 1), always_apply=True, p=1)])
...
# ---------------------------------- INITIAL TRANSFORMATION --------------------
     ...
     for i in range (1, Statistical_Telescope + 1):  # Photos have increased by a factor of 'Telescope_multiplicator'
       ...
       if i==1:
         ...
         img.imsave((path+'/'+filename),transformed_image) # Save the first rotated + crop image only  
...
def Classic_Augmentation_Super_Telescope (filename, path, Super_Telescope_multiplicator):
     ...
     for i in range (1, Statistical_Super_Telescope + 1):   # Photos have increased by a factor of 'Super_Telescope_multiplicator'
       ...
       if i==1:
         img.imsave((path+'/'+filename),image) # Saving the first image rotated + crop + centerCrop only
       ...
       # -----------------------------------
        ...
        for i in range (1, Statistical_Rectangle + 1):                  # Photos have increased by a factor of 'Rectangular_multiplicator'
         ...
         if i==1:
           ...
           img.imsave((path+'/'+filename),transformed_image)
         else:
...
def Balance_Dataset(path_in,path_out):
    ...
    print('Start Sharpening treatment on all classes.')
    print('\n')
    
    # Sharpness Transformation over all classes
    for i in range(0,Num_Classes):
      print(directory[i]+' class is processing...')
      Sharpness(path_in+'/'+(directory[i]),path_out+'/'+(directory[i]))
     
    print('\n')
    print('The scharpening process is finished.')
    print('\n')
    ...
    for i in range(0,Num_Classes):
      [rectangle,square,telescope,super_telescope]=Labeller(path_out+'/'+(directory[i]),path_out+'/'+(directory[i]),directory[i])
    ...
    for i in range(0,Num_Classes):
          ...
          for file in os.listdir(path_out+'/'+(directory[i])):
             filename = os.fsdecode(file)
             group = detect_group(filename)
    
             if group == 'R' or group == 'Q':
               Classic_Augmentation_Rectangle (filename, path_out+'/'+(directory[i]), Rectangle_multiplicator,directory[i], Larger_Classes, Too_many_rectangles)
    
             elif group == 'C':
               Classic_Augmentation_Telescope (filename, path_out+'/'+(directory[i]), Telescope_multiplicator)
    
             elif group == 'SC':
               Classic_Augmentation_Super_Telescope (filename, path_out+'/'+(directory[i]), Super_Telescope_multiplicator)  
        
    print('\n')
    for i in range(0,Num_Classes):
      print('The size of the '+directory[i]+' class is equal to '+str(counter(path_out+'/'+(directory[i]))))

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--path_in', help='input path of the entire dataset')
    parser.add_argument('--path_out', help='output path for saving the entire dataset')


    args = parser.parse_args()
    Balance_Dataset(args.path_in,args.path_out)
